...
class PID:
    ...
    def update(self, error, error_derivative=None):
        ...
        integralResult = self.K_i * self.integral
        print (f"integralresult{integralResult}")
        if abs(integralResult) > abs(self.integral_limit):
            if integralResult > 0:
                integralResult = abs(self.integral_limit)
            else:
                integralResult =  -1 *abs(self.integral_limit)

        output = (self.K_p * error) + (integralResult) + (self.K_d * derivative)
        print (f"K_p:{self.K_p * error} K_i: {integralResult} K_d {self.K_d * derivative}")
    ...
    def _get_integral(self, error, dt):
        ...
        # TODO: Calculate and return the integral term (DONE!)
