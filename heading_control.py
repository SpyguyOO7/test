...
def main():
    ...
    desired_heading = math.radians(desired_heading_deg)
    ...
    while True:
        ...
        if abs(yaw - previousYaw)   > math.pi:# logic to turn the 0 to 360 mapping into a infinite linear mapping
            if (yaw - previousYaw > 0):
                circles -= 1
            else:
                circles += 1
            
            print (f"round!{yaw - previousYaw} yaw: {yaw} previousyaw: {previousYaw}")

        previousYaw = yaw
        yaw =  (math.pi*2 * circles )+ yaw
